...
class SamplingMolecularMetrics(nn.Module):
    def __init__(self, dataset_infos, train_smiles):
        ...
        node_types = di.node_types
        edge_types = di.edge_types
        if len(node_types.shape) > 1:
            node_types = node_types.sum(0)
            edge_types = edge_types.sum(0)


        node_target_dist = node_types.type_as(self.generated_node_dist.node_dist)
        node_target_dist = node_target_dist / torch.sum(node_target_dist)
        self.register_buffer('node_target_dist', node_target_dist)

        edge_target_dist = edge_types.type_as(self.generated_edge_dist.edge_dist)
    ...
    def forward(self, molecules: list, name, current_epoch, val_counter, local_rank, test=False):
        ...
        except: print("Compute Custom metric failed")
