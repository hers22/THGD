...
class CoarsedTrainLossDiscrete(nn.Module):
    """ Train with Cross entropy"""
    def __init__(self, lambda_train):
        super().__init__()
        self.node_loss = CrossEntropyMetric()
        self.aug_loss = CrossEntropyMetric()
        self.edge_loss = CrossEntropyMetric()
        self.y_loss = CrossEntropyMetric()
        self.lambda_train = lambda_train

    def forward(self, masked_pred_X, masked_pred_X_aug, masked_pred_E, pred_y, 
                true_X, true_X_aug, true_E, true_y, log: bool):
        ...
        true_X_aug = torch.reshape(true_X_aug, (-1, true_X_aug.size(-1)))  # (bs * n, dx)
        true_E = torch.reshape(true_E, (-1, true_E.size(-1)))  # (bs * n * n, de)

        masked_pred_X = torch.reshape(masked_pred_X, (-1, masked_pred_X.size(-1)))  # (bs * n, dx)
        masked_pred_X_aug = torch.reshape(masked_pred_X_aug, (-1, masked_pred_X_aug.size(-1)))  # (bs * n, dx)
        masked_pred_E = torch.reshape(masked_pred_E, (-1, masked_pred_E.size(-1)))   # (bs * n * n, de)

        # Remove masked rows
        mask_X = (true_X != 0.).any(dim=-1)
        mask_X_aug = (true_X_aug != 0.).any(dim=-1)
        mask_E = (true_E != 0.).any(dim=-1)

        flat_true_X = true_X[mask_X, :]
        flat_true_X_aug = true_X_aug[mask_X_aug, :]

        flat_pred_X = masked_pred_X[mask_X, :]
        flat_pred_X_aug = masked_pred_X_aug[mask_X_aug, :]

        flat_true_E = true_E[mask_E, :]
        flat_pred_E = masked_pred_E[mask_E, :]

        loss_X = self.node_loss(flat_pred_X, flat_true_X) if true_X.numel() > 0 else 0.0
        loss_X_aug = self.aug_loss(flat_pred_X_aug, flat_true_X_aug) if true_X.numel() > 0 else 0.0
        loss_E = self.edge_loss(flat_pred_E, flat_true_E) if true_E.numel() > 0 else 0.0
        loss_y = self.y_loss(pred_y, true_y) if true_y.numel() > 0 else 0.0

        if log:
            to_log = {"train_loss/batch_CE": (loss_X + loss_X_aug + loss_E + loss_y).detach(),
                      "train_loss/X_CE": self.node_loss.compute() if true_X.numel() > 0 else -1,
                      "train_loss/X_aug_CE": self.aug_loss.compute() if true_X_aug.numel() > 0 else -1,
            ...
            if wandb.run:
                wandb.log(to_log, commit=True)
        return loss_X + self.lambda_train[0] * loss_X_aug + self.lambda_train[1] * loss_E + self.lambda_train[2] * loss_y

    def reset(self):
        for metric in [self.node_loss, self.aug_loss, self.edge_loss, self.y_loss]:
            metric.reset()

    def log_epoch_metrics(self):
        epoch_node_loss = self.node_loss.compute() if self.node_loss.total_samples > 0 else -1
        epoch_aug_loss = self.aug_loss.compute() if self.node_loss.total_samples > 0 else -1
        ...
        to_log = {"train_epoch/x_CE": epoch_node_loss,
                  "train_epoch/ring_CE": epoch_aug_loss,
