...
from rdkit.Chem import Crippen, QED, Descriptors
...
def check_stability(atom_types, edge_types, dataset_info=None, debug=False,atom_decoder=None):
...
def compute_molecular_metrics(molecule_list, train_smiles, dataset_info):
    ...
    if not dataset_info.remove_h:
        ...
        try:
            ...
            print(f'Analyze molecule stability failed')
...
def calculate_base_properties(mol):
    logP = Crippen.MolLogP(mol)
    qed = QED.qed(mol)
    sa_score = sascorer.calculateScore(mol)
    return logP, qed, sa_score

def count_large_rings(mol):
...
def penalized_property(mol):
    if mol is None:
        return None

    logP, qed, sa_score = calculate_base_properties(mol)

    y_m = logP - sa_score - count_large_rings(mol)
