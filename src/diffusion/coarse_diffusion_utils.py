...
def sample_discrete_features(probX, probX_aug, probE, node_mask, ring_constrain=False):
    ...
    probX_aug[~node_mask] = 1 / probX_aug.shape[-1]

    # Flatten the probability tensor to sample with multinomial
    probX = probX.reshape(bs * n, -1)       # (bs * n, dx_out)
    probX_aug = probX_aug.reshape(bs * n, -1)       # (bs * n, dx_out)

    # Sample X
    X_aug_t = probX_aug.multinomial(1)                                  # (bs * n, 1)

    if True:
        has_ring_1 = X_aug_t==1
        has_ring_2 = X_aug_t>=2
        min_node_required = torch.zeros_like(X_aug_t, dtype=torch.long)
        min_node_required[has_ring_1] = 3
        min_node_required[has_ring_2] = 4

        mask = torch.arange(dx).expand(bs * n, dx).type_as(min_node_required) < (min_node_required-1) # node count starting from 1
        # print(min_node_required-1)
        probX[mask] = 0
    ...
    X_aug_t = X_aug_t.reshape(bs, n)     # (bs, n)
    ...
    return PlaceHolder(X=X_t, X_aug=X_aug_t, E=E_t, y=torch.zeros(bs, 0).type_as(X_t))
...
def mask_distributions(true_X, true_X_aug, true_E, pred_X_aug, pred_X, pred_E, node_mask):
    """
    ...
    row_X[0] = 1.
    row_X_aug = torch.zeros(true_X_aug.size(-1), dtype=torch.float, device=true_X_aug.device)
    row_X_aug[0] = 1.
    ...
    true_X_aug[~node_mask] = row_X_aug
    pred_X[~node_mask] = row_X
    pred_X_aug[~node_mask] = row_X_aug
    true_E[~(node_mask.unsqueeze(1) * node_mask.unsqueeze(2) * diag_mask), :] = row_E
    pred_E[~(node_mask.unsqueeze(1) * node_mask.unsqueeze(2) * diag_mask), :] = row_E

    true_X = true_X + 1e-7
    pred_X = pred_X + 1e-7
    true_X_aug = true_X_aug + 1e-7
    pred_X_aug = pred_X_aug + 1e-7
    true_E = true_E + 1e-7
    pred_E = pred_E + 1e-7

    true_X = true_X / torch.sum(true_X, dim=-1, keepdim=True)
    pred_X = pred_X / torch.sum(pred_X, dim=-1, keepdim=True)
    true_X_aug = true_X_aug / torch.sum(true_X_aug, dim=-1, keepdim=True)
    pred_X_aug = pred_X_aug / torch.sum(pred_X_aug, dim=-1, keepdim=True)
    true_E = true_E / torch.sum(true_E, dim=-1, keepdim=True)
    pred_E = pred_E / torch.sum(pred_E, dim=-1, keepdim=True)

    return true_X, true_X_aug, true_E, pred_X, pred_X_aug, pred_E


def posterior_distributions(X, X_aug, E, y, X_t, X_aug_t, E_t, y_t, Qt, Qsb, Qtb):
    prob_X = compute_posterior_distribution(M=X, M_t=X_t, Qt_M=Qt.X, Qsb_M=Qsb.X, Qtb_M=Qtb.X)   # (bs, n, dx)
    prob_X_aug = compute_posterior_distribution(M=X_aug, M_t=X_aug_t, Qt_M=Qt.X_aug, Qsb_M=Qsb.X_aug, Qtb_M=Qtb.X_aug)   # (bs, n, dx)
    prob_E = compute_posterior_distribution(M=E, M_t=E_t, Qt_M=Qt.E, Qsb_M=Qsb.E, Qtb_M=Qtb.E)   # (bs, n * n, de)

    return PlaceHolder(X=prob_X, X_aug=prob_X_aug, E=prob_E, y=y_t)
...
def sample_discrete_feature_noise(limit_dist, node_mask):
    ...
    x_aug_limit = limit_dist.X_aug[None, None, :].expand(bs, n_max, -1)
    e_limit = limit_dist.E[None, None, None, :].expand(bs, n_max, n_max, -1)
    y_limit = limit_dist.y[None, :].expand(bs, -1)
    U_X = x_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max)
    U_X_aug = x_aug_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max)
    U_E = e_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max, n_max)
    U_y = torch.empty((bs, 0))

    long_mask = node_mask.long()
    U_X = U_X.type_as(long_mask)
    U_X_aug = U_X_aug.type_as(long_mask)
    U_E = U_E.type_as(long_mask)
    U_y = U_y.type_as(long_mask)

    U_X = F.one_hot(U_X, num_classes=x_limit.shape[-1]).float()
    U_X_aug = F.one_hot(U_X_aug, num_classes=x_aug_limit.shape[-1]).float()
    ...
    return PlaceHolder(X=U_X, X_aug=U_X_aug, E=U_E, y=U_y).mask(node_mask)
