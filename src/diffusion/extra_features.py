...
class ExtraFeatures:
    def __init__(self, extra_features_type, dataset_info):
        ...
        if extra_features_type in ['eigenvalues', 'all', 'freq']:
    ...
    def __call__(self, noisy_data):
        ...
        if self.features_type == 'cycles':
            ...
            test_tensor_x_cycles = torch.zeros_like(x_cycles)
            test_tensor_nonlcc_indicator = torch.zeros_like(nonlcc_indicator)
            test_tensor_k_lowest_eigvec = torch.zeros_like(k_lowest_eigvec)
            return utils.PlaceHolder(X=torch.cat((x_cycles, nonlcc_indicator, test_tensor_k_lowest_eigvec), dim=-1),
                                     E=extra_edge_attr,
                                     y=torch.hstack((n, y_cycles, n_components, batched_eigenvalues)))
        elif self.features_type == 'freq':
            eigenfeatures = self.eigenfeatures(noisy_data)
            E = noisy_data['E_t']
            extra_edge_attr = torch.zeros((*E.shape[:-1], 0)).type_as(E)
            n_components, batched_eigenvalues, nonlcc_indicator, k_lowest_eigvec, EigVals, EigVecs = eigenfeatures   # (bs, 1), (bs, 10),
                                                                                                # (bs, n, 1), (bs, n, 2)
...
class EigenFeatures:
    ...
    """
    def __init__(self, mode, max_freq=4):
    ...
    def __call__(self, noisy_data):
        ...
        if self.mode == 'eigenvalues':
            ...
            EigVals, EigVecs = eigvals[:,: self.max_freq], eigvectors[:, :, :self.max_freq]
            EigVals = EigVals.unsqueeze(1).expand(eigvals.shape[0],eigvals.shape[1],self.max_freq).unsqueeze(-1)
            # TODO: pad n < freq to NAN
            EigVecs = F.normalize(EigVecs, p=2, dim=2, eps=1e-12, out=None).unsqueeze(-1)
            ...
            nonlcc_indicator, k_lowest_eigenvector = get_eigenvectors_features(vectors=eigvectors,
                                                                               node_mask=noisy_data['node_mask'],
                                                                               n_connected=n_connected_comp)
            return n_connected_comp, batch_eigenvalues, nonlcc_indicator, k_lowest_eigenvector, EigVals, EigVecs
        else:
...
def get_eigenvectors_features(vectors, node_mask, n_connected, k=2):
    ...
    indices = torch.arange(k).type_as(vectors).long().unsqueeze(0) + n_connected    # bs, 1, k
    # indices = indices.expand(-1, n, -1)                                               # bs, n, k
    # first_k_ev = torch.gather(vectors, dim=2, index=indices)       # bs, n, k
    first_k_ev = vectors.transpose(-2,-1)[torch.arange(bs).unsqueeze(-1),indices].transpose(-2,-1)
    # first_k_ev = torch.stack([first_k_ev,first_k_ev],-1)
    # print(first_k_ev.shape)
