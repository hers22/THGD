...
def sample_discrete_features(probX, probE, X_type_mask, E_type_mask, node_mask):
    ...
    probX[X_type_mask==0] = 1 / probX.shape[-1]
    ...
    probE[E_type_mask==0] = 1 / probE.shape[-1]
...
def sample_discrete_features_with_validity_check(probX, probE, X_type_mask, E_type_mask, node_mask, max_valencies):
    ''' Sample features from multinomial distribution with given probabilities (probX, probE, proby)
        :param probX: bs, n, dx_out        node features
        :param probE: bs, n, n, de_out     edge features
        :param proby: bs, dy_out           global features.
...
def compute_masked_posterior_distribution(M, M_t, Qt_M, Qsb_M, Qtb_M, type_mask):
    ''' M: X or E
    ...
    dx = M_t.shape[-1]
    type_mask = type_mask.flatten(start_dim=1, end_dim=-1)  # bs x N
    type_mask_expanded = type_mask.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, dx, dx)

    Qt_M = torch.gather(Qt_M, 1, type_mask_expanded)   # bs, N, d_t-1, dt
    Qsb_M = torch.gather(Qsb_M, 1, type_mask_expanded) # bs, N, d0, d_t-1
    Qtb_M = torch.gather(Qtb_M, 1, type_mask_expanded) # bs, N, d0, dt

    Qt_M_T = torch.transpose(Qt_M, -2, -1)      # (bs, d, d)

    left_term = (M_t.unsqueeze(-2) @ Qt_M_T).squeeze()   # (bs, N, d)
    right_term = (M.unsqueeze(-2) @ Qsb_M).squeeze()     # (bs, N, d)
    product = left_term * right_term    # (bs, N, d)

    # denom = (M.unsqueeze(-2) @ Qtb_M).squeeze()      # (bs, N, d) @ (bs, d, d) = (bs, N, d)
...
def compute_masked_batched_over0_posterior_distribution(X_t, Qt, Qsb, Qtb, type_mask):
    """ M: X or E
        Compute xt @ Qt.T * x0 @ Qsb / x0 @ Qtb @ xt.T for each possible value of x0
        X_t: bs, n, dt          or bs, n, n, dt
        type_mask: bs, n        or bs, n, n
        Qt: bs, dm, d_t-1, dt
        Qsb: bs, dm, d0, d_t-1
        Qtb: bs, dm, d0, dt.
    """
    ...
    Qt = torch.gather(Qt, 1, type_mask_expanded)   # bs, N, d_t-1, dt
    Qsb = torch.gather(Qsb, 1, type_mask_expanded) # bs, N, d0, d_t-1
    Qtb = torch.gather(Qtb, 1, type_mask_expanded) # bs, N, d0, dt

    Qt_T = Qt.transpose(-1, -2)                 # bs, N, dt, d_t-1
    left_term = (X_t.unsqueeze(-2) @ Qt_T)      # bs, N, 1, d_t-1

    right_term = Qsb                            # bs, N, d0, d_t-1
    posterior_probs = left_term * right_term          # bs, N, d0, d_t-1

    denominator = posterior_probs.sum(-1, keepdim=True)
    denominator[denominator==0] = 1e-9 
    posterior_probs = posterior_probs / denominator
    # prod = (Qtb @ X_t.unsqueeze(-1)) # bs, N, d0, 1
    # denominator = prod           
...
def mask_distributions(true_X, true_E, pred_X, pred_E, X_type_mask, E_type_mask, node_mask):
    """
    ...
    true_X[X_type_mask==0] = row_X
    pred_X[X_type_mask==0] = row_X
    true_E[~(node_mask.unsqueeze(1) * node_mask.unsqueeze(2) * diag_mask), :] = row_E
    pred_E[~(node_mask.unsqueeze(1) * node_mask.unsqueeze(2) * diag_mask), :] = row_E
    true_E[E_type_mask==0] = row_E
    pred_E[E_type_mask==0] = row_E
...
def masked_posterior_distributions(X, E, X_type_mask, E_type_mask, y, X_t, E_t, y_t, Qt, Qsb, Qtb):
    prob_X = compute_masked_posterior_distribution(M=X, M_t=X_t, Qt_M=Qt.X, Qsb_M=Qsb.X, Qtb_M=Qtb.X, type_mask=X_type_mask)   # (bs, n, dx)
    prob_E = compute_masked_posterior_distribution(M=E, M_t=E_t, Qt_M=Qt.E, Qsb_M=Qsb.E, Qtb_M=Qtb.E, type_mask=E_type_mask)   # (bs, n * n, de)
...
def sample_masked_discrete_feature_noise(limit_dist, X_type_mask, E_type_mask, num_masks, node_mask):
    """ Sample from the limit distribution of the diffusion process"""
    bs, n_max = node_mask.shape
    x_limit = limit_dist.X.to(X_type_mask.device)
    e_limit = limit_dist.E.to(E_type_mask.device)
    x_limit = x_limit[X_type_mask.flatten()].reshape(bs, n_max, -1)
    e_limit = e_limit[E_type_mask.flatten()].reshape(bs, n_max, n_max, -1)
    y_limit = limit_dist.y[None, :].expand(bs, -1).to(X_type_mask.device)
    ...
    output = PlaceHolder(X=U_X, E=U_E, y=U_y)
    output.type_mask(X_type_mask, E_type_mask, encode_no_edge=False)
    output.mask(node_mask)
    return output
